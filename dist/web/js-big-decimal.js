...
function multiply_multiply(number1, number2) {
    ...
    number1 = stripTrailingZero_stripTrailingZero(number1.replace('.', ''));
    number2 = stripTrailingZero_stripTrailingZero(number2.replace('.', ''));
    ...
    * Formatting result
    */
    result = stripTrailingZero_stripTrailingZero(adjustDecimal(result, decimalLength));
    if (negative == 1) {
        result = '-' + result;
    }
    return result;
...
function divide_divide(dividend, divisor, precission) {
    // Convert to string
    ...
    return ((neg == 1) ? '-' : '') + trim(round_roundOff(quotent, precission - 2));
}
...
function subtract_subtract(number1, number2) {
    number1 = number1.toString();
    number2 = number2.toString();
    number2 = subtract_negate(number2);
...
function equals(left, right) {
    return (compareTo(stripTrailingZero_stripTrailingZero(left), stripTrailingZero_stripTrailingZero(right)) === 0);
}
function isExatclyZero(number) {
    return equals(stripTrailingZero_stripTrailingZero(abs_abs(number)), '0');
}
function isExatclyOne(number) {
    return equals(stripTrailingZero_stripTrailingZero(abs_abs(number)), '1');
}
...
function isAproxOne(number, precision = 8) {
    ...
    if (isExatclyOne(number))
        return true;
    if (compareTo_lessThan(abs_abs(subtract_subtract('1', number)), utils_tolerance(precision), true))
        return true;
    return false;
}
 ...
 * ```
 */
function pow(base, exponent, precision = undefined, negate = false) {
    ...
    const isBase10 = equals(abs_abs(base), '10');
    if (isBase10) {
        base = abs_abs(base);
        negate = !negate;
    }
    ...
    if (equals(abs_abs(base), '10')) {
        result = (negativeExponent) ? `0.${new Array(Number(abs_abs(exponentParts[0])) - 1).join('0')}1` : `1${new Array(exponentParts[0]).join('0')}`;
    }
    else {
    ...
    if (exponentSignificand) {
        ...
        //     // testworker.terminate();
        // }
        let tempBase = root10(abs_abs(base));
        for (let i = 0; i < exponentSignificand.length; i++) {
            fractionalExponent = multiply_multiply(fractionalExponent, pow(tempBase, exponentSignificand[i]));
...
function intPow(base, exponent, precision = 32) {
    ...
    let result = '1';
    while (compareTo_greaterThan(exponent, '0')) {
        if (isOdd(exponent)) {
            result = multiply_multiply(result, base);
        }
        base = multiply_multiply(base, base);
        exponent = divide_divide(exponent, 2).split('.')[0];
    }
    return result;
}
function nthRoot(x, n, precision = 8) {
    ...
    n = n.toString();
    pow_validate(n);
    if (compareTo_lessThan(n, '4', true)) {
        let guess = '1';
        ...
        let i = 0;
        while (true) {
            let newGuess = divide_divide(add_add(stripTrailingZero_stripTrailingZero(divide_divide(x, pow(guess, nMinusOne, precision + 2), precision + 2)), multiply_multiply(guess, nMinusOne)), n, precision + 2);
            ...
            if (compareTo_greaterThan(difference, lastDifference)) {
                ...
                return stripTrailingZero_stripTrailingZero(round_roundOff(guess, precision + 1));
            }
            if (compareTo_lessThan(difference, utils_tolerance(precision - 1))) {
                // console.log('newGuess exit under p')
                return stripTrailingZero_stripTrailingZero(round_roundOff(newGuess, precision + 1));
            }
        ...
        let i = 0;
        while (true) {
            let f0 = subtract_subtract(pow(x0, n, precision + 2), x);
            let f1 = subtract_subtract(pow(x1, n, precision + 2), x);
            let next = multiply_multiply(f1, divide_divide(subtract_subtract(x1, x0), subtract_subtract(f1, f0), precision + 2));
            x2 = subtract_subtract(round_roundOff(x1, precision + 2), round_roundOff(next, precision + 2));
            if (compareTo_lessThan(abs_abs(subtract_subtract(x2, x1)), utils_tolerance(precision + 1))) {
                return stripTrailingZero_stripTrailingZero(round_roundOff(x2, precision + 1));
            }
            if (sign(f0) !== sign(f1)) {
                x1 = divide_divide(add_add(x0 + x1), 2, precision + 2); // Switch to bisection method
            }
            x0 = x1;
            x1 = stripTrailingZero_stripTrailingZero(round_roundOff(x2, precision + 2));
            i++;
        }
...
const E = round_roundOff('2.718281828459045235360287471352662497757247093699959574966967627724076630353547594571382178525166427427466391932003059921817413596629043572900334295260595630738132328627943490763233829880753195251019011573834187930702154089149934884167509244761460668082264', 257);
...
function Euler(precision = 32) {
    ...
    while (true) {
        ...
        const next = divide_divide('1', f, precision + 2);
        if (compareTo_lessThan(abs_abs(next), utils_tolerance(precision))) {
            return round_roundOff(result, precision);
        }
...
function ln(x = 2) {
    ...
    let result = '0';
    let term = divide_divide(subtract_subtract(x, '1'), add_add(x, '1'), 33);
    let i = 0;
    while (true) {
        i++;
        let iteration = subtract_subtract(multiply_multiply('2', i), '1');
        let next = multiply_multiply(divide_divide('1', iteration, 33), pow(term, iteration));
        if (compareTo_lessThan(next, utils_tolerance(33))) {
            return round_roundOff(multiply_multiply('2', add_add(result, next)), 32);
        }
        result = add_add(result, next);
    }
}
function ln2(x = 2) {
    ...
    while (compareTo_greaterThan(x, '2', true)) {
        x = divide_divide(x, 2, 33);
        result = add_add(result, '1');
    }
    return round_roundOff(add_add(result, divide_divide(ln(x), LN2, 33)), 32);
}
function log(base) {
    base = base.toString();
    return round_roundOff(multiply_multiply(ln2(base), LN2), 32);
}
function log10(base) {
    base = base.toString();
    return divide_divide(log(base), LN10, 32);
}
...
function sin(x) {
    ...
    while (true) {
        ...
        if (compareTo_lessThan(abs_abs(next), utils_tolerance(33))) {
            result = add_add(result, next);
